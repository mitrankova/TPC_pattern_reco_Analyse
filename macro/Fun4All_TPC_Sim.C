...
//DST_TRKR_HITSET_piminus_24_phi_p0p2to1
//DST_TRKR_HITSET_piplus_24_phi_p0p2to1
void Fun4All_TPC_Sim(
    const int nEvents = 10,
    const int runnumber = 79513,
    const int segment = 0,
    const std::string outdir = ".",
    const std::string indir = "/sphenix/user/dcxchenxi/develope/macros/detectors/sPHENIX/output/fixed_piplus_pt0p3_z0_one_event_jobs/eta_p0p5_phi_0",
    const int nSkip = 0,
    const std::string collision = "run3pp",
    const std::string production = "ana532_nocdbtag_v001",
    const std::string& infilename = "DST_TRKR_HITSET_piplus_pt0p3_eta_p0p5_phi_0_z0_event00",
    const std::string& outfilename = "pi_plus_sim_eta_p0p5_phi_0")
{
 ...
 // std::string filename = indir+"/output_DST/"+infilename+"_"+to_string(runnumber)+"_"+to_string(segment)+".root";
   ...
   std::string filename = indir+"/"+infilename +".root";
  ...
  se->registerSubsystem(new TpcPolyClusterTrackReco()); 
