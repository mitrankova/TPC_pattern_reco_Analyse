...
namespace
{
                      ...
                      double& y_out)
  {
    ...
    double gprev = yprev_rot - Fitter::sagittaModel(xprev_rot, fit.S, fit.x0, fit.invR);
    ...
    for (int iscan = 1; iscan <= nscan; ++iscan)
    {
      ...
      const double gcur = ycur_rot - Fitter::sagittaModel(xcur_rot, fit.S, fit.x0, fit.invR);
      ...
      if (bracket)
      {
        ...
        for (int ib = 0; ib < 50; ++ib)
        {
          ...
          const double gm = ym_rot - Fitter::sagittaModel(xm_rot, fit.S, fit.x0, fit.invR);
  ...
  HardwareFitResult fit_hardware_track_points(const std::vector<InModuleTrackDisplay::HitPoint>& pts,
                                               const int fit_mode,
                                               ...
                                               const double weight_floor_frac)
  {
    ...
    std::vector<Fitter::FitPoint> layer_pad_points;
    std::vector<Fitter::FitPoint> layer_tbin_points;
    ...
    const Fitter::LineFit pad_fit = Fitter::fitLine(layer_pad_points);
    const Fitter::LineFit tbin_fit = Fitter::fitLine(layer_tbin_points);
    ...
    if (fit_mode == Fitter::FIT_SAGITTA)
    {
      result.pad_sagitta = Fitter::fitSagitta(layer_pad_points);
  ...
  FitResult fit_track_points(const std::vector<InModuleTrackDisplay::HitPoint>& pts,
                             const int fit_mode,
                             const double weight_power,
                             const double weight_floor_frac)
  {
...
InModuleTrackDisplay::~InModuleTrackDisplay()
{
...
int InModuleTrackDisplay::Init(PHCompositeNode*)
{
  ...
  {
    std::cerr << "InModuleTrackDisplay::Init - failed to load IdealPadMap from CDB"
              << std::endl;
    return Fun4AllReturnCodes::ABORTRUN;
  }

  m_outfile->mkdir("events");
  std::cout << "InModuleTrackDisplay::Init - writing display fits to "
            << m_outfilename << std::endl;
...
int InModuleTrackDisplay::End(PHCompositeNode*)
{
  if (m_outfile)
  {
    std::cout << "InModuleTrackDisplay::End - events seen: " << m_evt << std::endl;
    m_outfile->cd();
    m_outfile->Write();
    m_outfile->Close();
    delete m_outfile;
  ...
  std::cout << "InModuleTrackDisplay::End - wrote " << m_outfilename << std::endl;
  return Fun4AllReturnCodes::EVENT_OK;
}
...
bool InModuleTrackDisplay::get_nodes(PHCompositeNode* topNode)
{
  ...
  if (!m_tracks)
  {
    std::cerr << "InModuleTrackDisplay - could not find InModuleTrackContainer node" << std::endl;
    return false;
  }

  if (!m_hits)
  {
    std::cerr << "InModuleTrackDisplay - missing TRKR_HITSET" << std::endl;
    return false;
  }

  if (!m_idealPadMap)
  {
    std::cerr << "InModuleTrackDisplay - failed to create IdealPadMap" << std::endl;
    return false;
  }
...
double InModuleTrackDisplay::ideal_radius(const unsigned int layer) const
{
...
double InModuleTrackDisplay::ideal_phi(const int side,
